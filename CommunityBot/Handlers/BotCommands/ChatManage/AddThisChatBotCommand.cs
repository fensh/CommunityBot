...
namespace CommunityBot.Handlers.BotCommands.ChatManage
{
	public class AddThisChatBotCommand : BotCommandHandlerBase
	{
		...
		protected override async Task<IUpdateHandlerResult> HandleUpdateInternal(Update update, string inviteLink)
		{
			...
			string chatInviteLink = inviteLink?.Trim();

			if (!string.IsNullOrEmpty(chatInviteLink))
			{
				if (!chatInviteLink.StartsWith("https://t.me/joinchat/"))
				{
			...
			else
			{
				try
				{
					chatInviteLink = await _botClient.ExportChatInviteLinkAsync(chat.Id);
				}
			...
			await _chatRepository.AddOrUpdate(new SavedChat(chat.Id, chat.Title, chatInviteLink));
